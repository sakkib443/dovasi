...
import {
    ...
    FiImage, FiMonitor, FiCpu, FiCheck, FiInfo, FiCode, FiEdit3
} from 'react-icons/fi';
...
export default function EditWebsitePage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const [catRes, assetRes] = await Promise.all([
                fetch(`${BASE_URL}/categories/admin/all?type=website`, {
    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            if (response.ok) {
                alert('Website template updated successfully! ✅');
            ...
            <form className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                < div className="lg:col-span-4 space-y-8 sticky top-24" >
                    ...
                    <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm">
                        <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-4">Quick Stats</h3>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl">
                                <p className="text-[10px] text-slate-500 mb-1">Total Sales</p>
                                <p className="text-lg font-black text-slate-800 dark:text-white">{websiteData?.salesCount || 0}</p>
                            </div>
                            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl">
                                <p className="text-[10px] text-slate-500 mb-1">Total Likes</p>
                                <p className="text-lg font-black text-slate-800 dark:text-white">{websiteData?.likeCount || 0}</p>
                            </div>
                        </div>
                    </div>
                </div>
